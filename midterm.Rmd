...
reg.cv <- 
  train(sale_price ~ ., data = st_drop_geometry(Philly_Housing_joined) %>% 
...
### Moran's I test
```{r}
coords <- st_coordinates(Philly_Housing_joined) 

neighborList <- knn2nb(knearneigh(coords, 5))

spatialWeights <- nb2listw(neighborList, style="W")

Philly_Housing_joined$lagPrice <- lag.listw(spatialWeights, Philly_Housing_joined$sale_price)
...
```{r}
Price_to_predict <- Philly_price_all %>%
  filter(toPredict == "CHALLENGE")

Price_to_predict %>%
  mutate(predicted = predict(reg.training, Price_to_predict))
```
...
>>>>>>> 07086121716c87a8c67bdb81129d737654e97c80
