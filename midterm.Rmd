...
neighborhood <- st_read("C:/Users/Jingyi's lenovo/Desktop/Penn/Fall 2023/MUSA 5080/midterm/Neighborhoods_Philadelphia.geojson") %>%
  st_transform('ESRI:102728')
...
blockgroup21 <- 
  ...
  mutate(pctForeign = ifelse(TotalPop > 0, ForeignBorn / TotalPop, 0),
         ...
         pctAfrican = ifelse(TotalPop > 0, AfricanAmerican / TotalPop, 0),
         year = "2021") %>%
  dplyr::select(-ForeignBorn,-MobilityDifferentHouse,-MobilityTotal,-TotalPop, -AfricanAmerican)
```
...
Third, we did a Moran's I test in moran.mc function with neighbors of 5 to test the spatial autocorrelation and check the cluster pattern of the house prices. The plot shows the value of Moran's I and the frequency of 999 random permutated I. A scatterplot of spatial lag of errors and prediction errors is also displayed. It is used to show if the spatial autocorrelation in the errors exist. With the above model, we put our unpredicted data into the regression model and showed the results in a map.

The last part of the analysis is the generalizability of neighborhood model, which is to test the generalizability across the space. To achieve this, we first ran another regression by each neighborhood and created a map to visualize the mean absolute percent errors by neighborhood. We also did a testing using the Census data of African American residents to categorize the neighborhoods and making tables to compare the mean absolute percent errors.

## Results

### Training set linear model summary results  
Table 3.1 summarizes the results of the linear model when we ran the OLS regression algorithm. The total amount of observations is 14272 from the training dataset. Our dependent variable is the house sale price, and each variable below is the predictor for the house sale price. The constant is 299768.7, which is the value of house price when the value of every variable is 0. The number in the second column for each variable row is the coefficient, and the positive or negative number decides the increase or the decrease of the sale price when the variable increases one unit. For example, as house age increases one year, the sale price will decrease $86.689. The number in the parentheses is the standard errors for each coefficient, and smaller errors indicates that the coefficient is more reliable. The total area of the house has the standard error of 0.453, but that of mobility percent is 14,884.300, which means that the coefficient of the total area is more precise than the mobility percentage.

The stars in the upper right of each coefficient represents the statistical significance level, which uses p-value to measure, and it gives the information of the probability to reject the null hypothesis that there is no relationship between the house sale price and each predictor. Total area, interior condition, shooting victims, distance to commercial corridors, distance to parks, median household income, foreign-born percentage and mobility rate all have a p-value less than 0.01, meaning that these variables play a significant role in predicting the sale price. Predictor distance to school has a p-value less than 0.05, while house age has a p-value larger than 0.1. Therefore, these two play a less significant role and are not a significant predictor of the housing prices.

The R-square and adjusted R-square is 0.415 and 0.414, which indicates the measure of how well the model fits the data. In our model, the variables explain 41.5% of the variability in the house prices can be explained by the predictors we have. This value is an intermediate fit for the regression model.

The F-statistic is used to test the hypotheses about the variances in the data. Its value in our model is 1,009.786, and it has a p-value smaller than 0.01, which indicates that the overall model is statistically significant and at least one predictor is significantly related to the housing prices.
...
We have the mean absolute error of 102962, meaning that the predicted prices are 102962 less or larger than the actual prices on average, and the error takes 40% of the whole predicted prices. While the MAE and MAPE refer to the validation of the fitness of the model, we estimate that our model is not problematic, but it does not give a high fitness as well. Therefore, we need a further validation approach.
...
We chose African American as a race context to test the generalizability across urban contexts. Figure 3.11 shows the distribution of the majority African-American and Majority non African-American block groups. The north Philadelphia is more segregated with more African-American groups, and the northwest Philadelphia is more segragated with non African-American groups. The center Philadelphia is integrated with both majorities. Table 3.12 calculated the mean absolute errors in baseline regression model and the neighborhood-effect regression. There is not a lot difference in the goodness of fit across the races, but the gap is larger for the neighborhood effects model, which is 10% compared with 6%.
...
## Conclusion  
In conclusion, in order to achieve our primarily goal on understanding the factors influencing housing prices in Philadelphia, we used data from OpenDataPhilly and went through all kinds of analysis including using maps, charts, and regressions. The results of our analysis went though testing by validation, accuracy and generalizability. 
  Data preparing was done through a major of three steps: collecting, selecting, and manipulating. Collecting and selecting is a very subjective process, for it is highly depending on what we think might be influencing the housing price. However, manipulation of the data needs to be done through approaches that can best highlight the characteristic of influential factors. For example, to better assess the impact of exterior environment on housing prices, we employed distinct approaches on factors we selected for analysis. For shooting incidents data, we focused on the number of victims who suffered head gunshot wounds within a 1000-foot radius of each house. This approach aimed to capture the most critical injuries affecting safety. To gauge the proximity of houses to schools, commercial corridors, and public parks, we calculated Euclidean distances, emphasizing the convenience of living in each house. Additionally, we incorporated census variables, linking block group data to individual houses to study the demography that can impact housing value. 
  In our analysis, we present a table summarizing various independent variables that might impact housing prices. The table includes statistics for interior house condition, demographic characteristics, and exterior environment. Notably, we identify high variability, especially in some variables, as evidenced by the standard deviations. Additionally, we consider potential outliers in the dataset.
  Moreover, we display a correlation matrix illustrating the relationships between numeric variables. While many pairs show no or slight correlation, some, like the distance to schools and distance to parks, exhibit strong positive correlations with housing price.
We also analyze scatterplots to explore the linear relationship between housing prices and the factors, such as median household income, house age, and total area. These visualizations reveal trends such as the relationship between housing prices and total area, where larger houses tend to command higher prices. However, the strong positive factors such as distance to parks show weaker linear association on scatterplot.
  Furthermore, we present a choropleth map showing the distribution of housing prices by block groups in Philadelphia. The map demonstrates that most neighborhoods have relatively low prices, except for some in the northwest and downtown areas. And we create maps to highlight specific independent variables of interest. These include maps of geographic mobility, school density, and shooting incidents with head injuries, which offer insights into their spatial distribution and impact on housing prices. And maps of our variables suggests that northwest and downtown areas have relatively high mobility rate, medium distance to schools, and more frequent shooting incident with head injuries.
  Our methods encompassed OLS regression in R, utilizing packages such as kable, ggplot, and stargazer. We separated the data into training and test sets, employed cross-validation, and assessed goodness of fit using metrics like mean absolute error (MAE) and mean absolute percent error (MAPE). We also incorporated spatial autocorrelation of Moran I to explore the degree of spatial clustering or dispersion of the variables across a geographic area.
  Results indicate that the regression model explains approximately 41.5% of the variability in housing prices. The F-statistic, with an overall p-value less than 0.01, suggests that the overall model is statistically significant. Some variables, such as total area, interior condition, shooting incidents, and median household income, have low p-values, indicating their significance in predicting housing prices. In contrast, house age and distance to schools have less significance.
  Cross-validation results revealed a low mean Root Mean Square Error (RMSE) of 180808.5 which indicates that our model is not capable to provide the most accurate prediction of housing prices. The MAE is with higher frequencies in the 90000-110000 range and failed to be generalizable to the new data. We observed a scattered distribution in residual plots-- a well-fit model, indicating some variation in the model's performance. Additionally, a Moran's I test showed a moderate positive spatial autocorrelation in housing prices. We also explored the generalizability of neighborhood models and the influence of race on model performance. The Mean Absolute Percentage Error (MAPE) by neighborhood showed that the neighborhood effects model exhibited consistent accuracy across neighborhoods, while the baseline regression displayed higher MAPE in specific areas.
  In summary, our analysis provides insights into the factors influencing housing prices in Philadelphia and highlights the strengths and limitations of our predictive models. Further research and refinement of the models may be necessary to achieve higher accuracy and predictive power.
